# 👋Hi, I'm Lucian!
## **Sophomore @ Central China Normal University | Statistics Major**
I am a statistics student driven by a passion for Medical AI, LLM Safety and Bioinformatics. Currently, I am building a solid foundation in math and exploring the vast potential of machine learning in life sciences and AI safety.

🛠 **Tech & Math Stack**
- 🔭 **Focusing on**: Medical AI, LLM Safety & Reasoning, Bioinformatics, and Single-cell Omics.
- 💻 **Tools & Languages**: Python (ML Ecosystem) and R.
- 🏆 **Major Achievement**: First Prize in the Contemporary Undergraduate Mathematical Contest in Modeling (CUMCM).
- 🌱 **Learning Path**: Deepening my expertise in math while exploring how to build robust, interpretable, and safe LLMs for critical clinical applications.

🎯 **Current Goal**: 
- Actively looking for research internships to bridge the gap between statistical theory and safe medical AI applications.
- Actively exploring the intersection of Statistics and ML to find my unique niche
- Aspiring to be a proficient machine learning engineer.

🏃 **Beyond the Screen**
When I'm not debugging, fine-tuning models or proving theorems, you'll find me on the track.
- **10km PB**: 40:18
- **Half Marathon**: 1:37:42
Running taught me that research, like a marathon, is about persistence and pacing.

📫 **Reach out**: 2649173288@qq.com | Always open to research and internship opportunities!

